...
class BATP_details_notification extends StatefulWidget {
  ...
  @override
  State<BATP_details_notification> createState() =>
      _BATP_details_notificationState();
}

class _BATP_details_notificationState extends State<BATP_details_notification> {
  Future<List<BatpDetailsModel>>? futurePost;
  String rejectNote = " ";

  Future<List<BatpDetailsModel>> fetchPostdetails() async {
    var response = await http.post(
        Uri.parse(ConstApiLink().pendingBATPDetailsApi),
    ...
    if (response.statusCode == 200) {
      ...
      return parsed
          .map<BatpDetailsModel>((json) => BatpDetailsModel.fromJson(json))
          .toList();
    } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Container(
        ...
        child: FutureBuilder<List<BatpDetailsModel>>(
          future: futurePost,
                  ...
                  Row(
                    ...
                    children: [
                      TextButton(
                        ...
                        onPressed: () async {
                          var response = await http.post(
                              Uri.parse(ConstApiLink().pendingBATPApproveApi),
